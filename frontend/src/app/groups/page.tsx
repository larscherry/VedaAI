      ...
      <main className="flex-1 lg:bg-[var(--card)] lg:rounded-2xl lg:flex lg:flex-col lg:overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-7 pt-4 sm:pt-6 pb-32 lg:pb-32" style={{ background: "linear-gradient(to bottom, var(--muted), var(--card))" }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-[var(--foreground)]">My Groups</h1>
              <p className="text-sm text-[var(--muted-foreground)] mt-1">Manage your class groups and student rosters.</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="mt-20 flex flex-col items-center justify-center text-center">
              ...
              <p className="mt-4 text-sm text-[var(--muted-foreground)]">No groups yet</p>
              <button
                onClick={() => setShowCreate(true)}
                className="mt-4 inline-flex items-center gap-2 rounded-full bg-[var(--foreground)] text-[var(--background)] text-sm font-semibold px-5 py-2.5 hover:opacity-90 transition cursor-pointer"
              >
            ...
            <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-5">
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      <img src="/profile.png" alt={g.name} className="h-8 w-8 sm:h-10 sm:w-10 rounded-full object-cover shrink-0" />
                      <div className="min-w-0">
                        <h3 className="text-sm sm:text-lg font-bold text-[var(--foreground)] truncate">{g.name}</h3>
                        {g.description && (
                          <p className="text-xs text-[var(--muted-foreground)] truncate">{g.description}</p>
                        )}
                      </div>
                    </div>
                    <button
                  ...
                  <div className="mt-3 sm:mt-6 flex items-center gap-4 text-xs sm:text-sm text-[var(--muted-foreground)]">
              ...
              <div
                onClick={() => setShowCreate(true)}
                className="bg-[var(--card)] rounded-2xl border-2 border-dashed border-[var(--border)] p-4 sm:p-6 flex items-center justify-center hover:border-[#E94E1B] hover:bg-[#fef2ee] transition cursor-pointer"
              >
                <div className="text-center">
                  <Plus className="h-6 w-6 text-[var(--muted-foreground)] mx-auto" />
                  <p className="mt-2 text-sm font-medium text-[var(--muted-foreground)]">Create New Group</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
            <div className="bg-[var(--card)] rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowCreate(false)} className="p-1 cursor-pointer"><X className="h-5 w-5 text-[var(--muted-foreground)]" /></button>
              </div>
              <input
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                placeholder="Group name"
                className="w-full h-11 rounded-xl bg-[var(--hover)] border border-[var(--border)] px-4 text-sm outline-none focus:ring-2 focus:ring-[var(--brand)]/30 mb-3"
              />
              <input
                value={newDesc}
                onChange={(e) => setNewDesc(e.target.value)}
                placeholder="Description (optional)"
                className="w-full h-11 rounded-xl bg-[var(--hover)] border border-[var(--border)] px-4 text-sm outline-none focus:ring-2 focus:ring-[var(--brand)]/30 mb-4"
              />
              <button onClick={handleCreate} className="w-full h-11 rounded-full bg-[var(--foreground)] text-[var(--background)] text-sm font-semibold hover:opacity-90 transition cursor-pointer">
                Create Group
              </button>
            </div>
          </div>
